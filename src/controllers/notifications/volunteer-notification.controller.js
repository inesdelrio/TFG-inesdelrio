...
async function markVolunteerNotificationAsUnread(req, res, next) {
  try {
...
async function openVolunteerNotificationEvent(req, res, next, dependencies = {}) {
  const markAsRead =
  ...
  try {
    ...
    return res.redirect(
      `/eventos/${Number(req.params.eventId)}?backToNotifications=1`,
    );
  } catch (error) {
    if (error.code === "NOTIFICATION_NOT_FOUND") {
      return res.status(404).render("pages/errors/404", {
...
module.exports = {
  ...
  markVolunteerNotificationAsUnread,
